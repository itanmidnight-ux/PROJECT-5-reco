...
check_existing_database() {
  echo "Detectando bases de datos existentes..."
  ...
  if run_as_postgres psql -tAc "SELECT 1 FROM pg_database WHERE datname='${DB_NAME}'" 2>/dev/null | grep -q 1; then
    ...
    echo "  [D] Eliminar y recrear la base de datos"
    echo "  [K] Mantener la base de datos existente"
    echo "  [A] Eliminar TODAS las bases de datos y recrear"
    echo ""
    read -p "Selecciona una opción [D/K/A]: " -n 1 -r
    echo ""
    
    case "${REPLY}" in
      D|d)
        echo "Eliminando base de datos ${DB_NAME}..."
        run_as_postgres psql -c "DROP DATABASE IF EXISTS ${DB_NAME};" 2>/dev/null || true
        run_as_postgres createdb --owner="${DB_USER}" "${DB_NAME}"
        echo "✅ Base de datos recreada."
        ;;
      A|a)
        echo "Eliminando todas las bases de datos..."
        run_as_postgres psql -c "DROP DATABASE IF EXISTS ${DB_NAME};" 2>/dev/null || true
        run_as_postgres psql -c "DROP ROLE IF EXISTS ${DB_USER};" 2>/dev/null || true
        run_as_postgres psql -c "CREATE ROLE ${DB_USER} LOGIN PASSWORD '${DB_PASSWORD}';" 2>/dev/null || true
        run_as_postgres createdb --owner="${DB_USER}" "${DB_NAME}"
        echo "✅ Base de datos y usuario recreados."
        ;;
      K|k|*)
    ...
    echo "No se detectó base de datos existente. Creando nueva..."
    if ! run_as_postgres psql -tAc "SELECT 1 FROM pg_roles WHERE rolname='${DB_USER}'" 2>/dev/null | grep -q 1; then
      run_as_postgres psql -c "CREATE ROLE ${DB_USER} LOGIN PASSWORD '${DB_PASSWORD}';" >/dev/null 2>&1 || true
    fi
    run_as_postgres createdb --owner="${DB_USER}" "${DB_NAME}" 2>/dev/null || true
  fi
}
...
ensure_role_and_database() {
  ...
  run_as_postgres psql -d "${DB_NAME}" -c "ALTER SCHEMA public OWNER TO ${DB_USER};" >/dev/null 2>&1 || true
  run_as_postgres psql -d "${DB_NAME}" -c "GRANT ALL ON SCHEMA public TO ${DB_USER};" >/dev/null 2>&1 || true
  run_as_postgres psql -d "${DB_NAME}" -c "GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO ${DB_USER};" >/dev/null 2>&1 || true
  run_as_postgres psql -d "${DB_NAME}" -c "GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA public TO ${DB_USER};" >/dev/null 2>&1 || true
}
